# vpc variable values 

vpc_cidr_block = "10.1.0.0/16"
instance_tenancy = "default"
vpc_name = "Prod-Env-Yashwanth-vpc"

# public subnet variable values 

pub_sub_cidr_block = "10.1.1.0/24"
pub_sub_az = "us-east-2a"
pub_sub_name = "Prod-Env-Yashwanth-pub-subnet"

# private subnet variable values
pvt_sub_cidr_block = "10.1.2.0/24"
pvt_sub_az = "us-east-2b"
pvt_sub_name = "Prod-Env-Yashwanth-pvt-subnet"

# Internet Gate Way variables 
igw_name = "Prod-Env-Yashwanth-igw"

# Public Route table
pub_rt_cidr_block = "0.0.0.0/0"
pub_rt_name = "Prod-Env-Yashwanth-pub-rt"

# Private Route table
pvt_rt_name = "Prod-Env-Yashwanth-pvt-rt"
pvt_rt_cidr_block = "0.0.0.0/0"

# Elastic IP
eip_name = "Prod-Env-Yashwanth-eip"

# NAT Gateway
nat_gw_name = "Prod-Env-Yashwanth-gw-NAT"

# Key Pair
keypair_name = "Prod-Env-yashwanth-keypair-new"
pub_key      = "ssh-rsa  chada@YASH"

# Security Group
sg_name        = "Prod-Env-Yashwanth-sg"
sg_description = "Allow TLS inbound traffic and all outbound traffic"
ssh_port  = 22
port_8080 = 8080
port_9000 = 9000
ingress_cidr_blocks = ["0.0.0.0/0"]
egress_cidr_blocks  = ["0.0.0.0/0"]

# EC2 Instances
ami_id        = "ami-06e3c045d79fd65d9"
instance_type = "t2.micro"

pub_instance_name  = "Prod-Env-Yashwanth-public-instance"
pvt_instance_name  = "Prod-Env-Yashwanth-private-instance"
pub_instance_count = 1
associate_public_ip = true

prod_env_region = "us-east-2"